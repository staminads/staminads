...
interface ExploreTemplatesProps {
  onSelectTemplate: (dimensions: string[]) => void
  customDimensionLabels?: CustomDimensionLabels | null
}

export function ExploreTemplates({ onSelectTemplate, customDimensionLabels }: ExploreTemplatesProps) {
  ...
  const handleTemplateClick = (template: ExploreTemplate) => {
    if (template.title === 'Custom report') {
      ...
      onSelectTemplate(template.dimensions)
    }
  }
